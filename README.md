...
предупреждения. Устаревший JSONB `payload` удаляется миграцией `000003` после
проверки, что его строки уже перенесены в `file_rows`.
...
Миграция `000003_drop_legacy_file_payload` удаляет старую копию файла из
`uploaded_files.payload`. Перед удалением миграция проверяет, что строки уже
присутствуют в `file_rows`, и останавливается при риске потери данных.
...
`uid`, атомарное исправление строки, отсутствие `data` и удаление legacy `payload`.
