...
│   └── migrations/                 # последовательные SQL up-миграции
...
`go:embed`. Начальная миграция сразу создаёт актуальную схему без legacy-колонок
и переходных типов. Следующие изменения схемы должны добавляться новыми файлами.
...
Миграции выполняются только вперёд. Откат production-схемы выполняется новой
исправляющей миграцией, а восстановление данных — из проверенной резервной копии.
...
Тест проверяет чистую схему, генерацию внутреннего `SERIAL id` и публичного `uid`,
разрешение конфликтов без накопления истории и атомарное исправление строки.
