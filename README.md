...
3. Предпросмотр и экспорт рассылки получают `fileId`, восстанавливают выбранный
   файл через `GetFileData` и формируют сообщения из строк `file_rows`. Оперативная
   память используется только на время конкретного HTTP-запроса.
4. `POST /api/contacts/save` отдельно переносит валидные строки выбранного файла в
   общий справочник `contacts`. Он нужен для дедупликации и конфликтов, но сейчас
   не является источником данных для рассылки.
5. Каждый загруженный файл имеет собственный UUID. Фронтенд работает с последним
   загруженным `fileId`; автоматического объединения нескольких файлов в одну
   рассылку сейчас нет.
